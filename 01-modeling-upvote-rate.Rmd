...
The two kinds of rating users can leave for posts are upvotes and downvotes.
...
Yet another way of thinking about the upvote rate is as the probability of a
vote being an upvote.
...
concentrated around fewer values which means we gain make be more and more
certain about our beliefs about the true upvote rate.
...
What this would result in are very erratic estimates in the beginning which
would only smooth out over time.
...
If we were to use this metric to compare posts, getting a very high estimate
would essentially come down to luck in the early stages of a post.
It might then temporarily fare overly well (or overly poorly) compared to other
posts.
We have to take into account prior information and context to avoid this.
...
collected 10 data points which amounted to an upvote rate of exactly 0.6.
...
average (indicated by light grey lines, the priors chosen here are 0.1 through
0.9 and the weight is kept constant at 20):
...
And here is how this would look like with a weight of 100:
...
How to properly seed the Bayesian average is still an open question.
For now, we will chose a "common sense" global prior that will be used for all
posts.
