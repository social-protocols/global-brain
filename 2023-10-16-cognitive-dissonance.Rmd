...
$H(p,q)$ is the cross entropy between Bob and Alices's probabilities: Bob's estimate of Alice's average error. The closer the estimate $q$ is to what Bob thinks is the more correct probability $p$, the lower the expected error. Cross entropy is minimized when $p=q$, at which point $H(p,q)=H(p,p)=H(p)$.
...
$H(p,q)$ will always be greater than $H(p)$. Bob measure of Alice's error will always be greater than his measure of his own error as long as his beliefs differ from Alice's. 

This is due to a very powerful property of surprisal: in the long term, surprisal is minimized when the estimated probability equals the actual frequency of the event. Over- or under- estimating the probability always results in more error in the long run. So if you truly think 90% of votes will be upvotes, then you will minimize your expected error by estimating the probability of an upvote to be 90%.

$H(p,q)$ is always greater than $H(p,p)=H($p)$ because $H(p,q)$ tells us what Alice's average error *would* be, in the long term, if $p$ were the true frequency. The closer $q$ is to $p$, the smaller the expected average error.

Another way of looking at this is to realize Bob will always expect Alices's average error to be greater than his own, as long as he thinks $p$ is a better estimate than $q$. Presumably, Bob thinks he knows something Alice doesn't know. If Bob thought Alices's estimate $q$ was better than his own estimate $p$, Bob would change his estimate to $q$!

## Total Relative Entropy = Cognitive Dissonance

The difference between $H(p,q)$ and $H(p)$ is **relative entropy**, also known as the Kullback–Leibler divergence or KL-divergence. It can be thought of as "how much **more** surprised I expect Alice to be than Bob". Since $H(p,q) >= $H(p)$, relative entropy is that **it is never negative**, regardless of whether $p$ is greater or less than $q$.
...
Note also how the rate of reduction of cognitive dissonance falls as $q$ approaches $p$.
