...
If $y_i$ are users votes, and $ŷ_i$ is always equal to the uninformed users prediction $q$, then our cost function is identical to the cost function used when training a neural network:
...
The global brain can't eliminate all cognitive dissonance -- it can only reduce it as much and as fast as possible. 
...
Once we know how much attention we are spending when we show posts to users in different positions, we can calculate how much cognitive dissonance is reduced, per unit of attention spent, by showing users various post/note combinations. We'll call the rate of reduction of entropy per unit of attention the **information rate**.


## Vote Rate

The first step in calculating the information rate is to estiamte the **vote rate** of a post -- the number of votes it gets per unit attention.
