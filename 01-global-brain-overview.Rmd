# Introduction to the Global Brain
...
There are three fundamental interaction and visual presentation elements users interface with:
...
1) a vote reflects a users intent to give a post more/less attention (see [The Law of Attention](https://jonathanwarden.com/the-law-of-attention)) and
2) this intent is caused by underlying beliefs that we cannot observe (latent variables -- see section below).

By making some reasonable assumptions about the causal relationship between beliefs and votes and then watching how exposure to notes causes changes to votes, we can model these underlying beliefs and predict how exposure to a note will cause changes to these beliefs, which will cause changes to other beliefs, and so on. We describe our model in more detail below.

## Example: Did an Earthquake Just Happen?

Suppose there is a post with a video showing the aftermath of a recent earthquake. Let's call it post A. This post goes viral, but then somebody posts a note B on the video which says "This video is actually from an earthquake in 2004. Here is a link to the original video.".
...
## Cognitive Dissonance in the Global Brain
