...
They have a root post which can have any number of replies, and each reply can have any number of replies, and so on.
...
It discourages following threads for longer than a few levels because coherence is lost in any thread very quickly.
...
- **Breadth over depth**: Threads are hard to follow because coherence is lost in the thread as soon as there are several replies to a post/reply. This is especially true for longer threads. There is a lot less friction scrolling through the direct replies of any particular post/reply, rather than following the discussion thread more deeply.
- **Sensitivity to social cues**: This setup is prone to users being influenced by social cues and creates a favorable environment for opinion being swayed by majority sentiment rather than good arguments. Phenomena such as [astroturfing](https://en.wikipedia.org/wiki/Astroturfing) and [dogpiling](https://en.wikipedia.org/wiki/Dogpiling_(Internet)) flourish in such an environment because they *work*.
- **Unchecked misinformation**: As people often gloss over threads getting a brief impression of what other people are thinking about a certain piece of media, misinformation often remains unchecked. You can just post a piece of false information as a reply (or a post for that matter) and it is likely read much more often than checked. Rebuttals cannot keep pace with misinformation.
...
The post's parents are displayed above the post so that the context of the conversation is not lost.
The parent thread is displayed when the post is not a top-level post, i.e., the root of the discussion.
...
The coherence in a discussion thread is preserved and following a discussion thread almost looks like reading a chat log.
