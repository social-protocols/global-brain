...
- $C \sim \text{Gamma}(2, 2)$
- $m_{\text{global}} \sim \text{Beta'}(m, C+2)$
- $m_{\text{post}} \sim \text{Beta'}(m_{\text{global}}, C_{\text{global}})$

We use a Beta distribution parameterized using mean and sample size instead of $α$ and $β$.

$$
  \text{Beta'}(m, C) = \text{Beta}(m \times C, (1 - m) \times C)
$$


We can easily convert between (α,β) and (mean, sampleSize) using the formulas:
...
Which gives us the formula for the Bayesian Average.
